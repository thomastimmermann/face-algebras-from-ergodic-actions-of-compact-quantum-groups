...
comultiplication. Denote by $\Delta_{rs}^{\op}$ the composition of
$\Delta_{rs}$ with the flip map.

\begin{Cor}
Let $\mathscr{A}$ be a partial Hopf algebra. Then the total antipode $S:A\rightarrow A$ is an antihomomorphism and $\Delta_{rs}(S(a)) = (S\otimes S)\Delta_{sr}^{\op}(a)$ for all $a\in A$.
\end{Cor} 
