...
\begin{Cor} \label{corollary:antipode}
...
for all $a,b\in A$. In particular, $S(\UnitC{k}{m})=\UnitC{m}{k}$ for
all $k,m\in I$ if
$S$ is bijective.
\end{Cor} 
...
\end{align}
a \emph{left} (or  \emph{right}) \emph{integral} if
...
&= \phic{k}{m}(a)
  \UnitC{k}{l} &&\big(\text{or} &
  (\phic{k}{l} \otimes
  \id)(\Delta_{l,l}(a))&= \phic{k}{m}(a) \UnitC{l}{m}\big)
\end{align}
 for all $k,l,m\in I$, $a \in A\pmat{k}{k}{m}{m}$. If $\phi$ is a left
 and a right integral, we just call it an \emph{integral}.
\end{Def}
\begin{Lem}
  \begin{enumerate}
  \item If $\phi$ is a left or right integral, then
    $\phic{k}{m}(\UnitC{k}{m})=1$ for all $k,m\in I$.
  \item If a left integral and a right integral exist, then the two
    are equal and unique.
  \item Assume that $\mathscr{A}$ is an $I$-partial Hopf algebra with
    bijective antipode $S$.  If $\phi$ is a left or right integral,
    then it is unique, an integral, and equal to $\phi\circ S$.
    \end{enumerate}
\end{Lem}
\begin{proof}
To see 1., take     $a=\UnitC{k}{k}$ in \eqref{eq:integral}. For 2.,
assume that $\phi$ and $\psi$ are a  left and a
 right integral.     Then for all $k,l,m\in I$, $a\in A\pmat{k}{k}{m}{m}$,
    \begin{align*}
      \phic{k}{m}(a)  &= (\Grt{\psi}{k}{l} \otimes
      \phic{l}{m})(\Delta_{l,l}(a)) = \Grt{\psi}{k}{m}(a).
    \end{align*}
To prove 3., assume that $\phi$ is a left or right integral. Then
Corollary \ref{corollary:antipode} implies that $\phi\circ S$ is a
right or left integral, and 2.\ implies the claim.
\end{proof}
...
\begin{Def} A \emph{partial compact quantum group} $\mathscr{G}$ is a
  partial Hopf $^*$-algebra $\mathscr{A} = P(\mathscr{G})$ with a
left and right integral  $\varphi$ that is positive in the sense  that $\varphi(a^*a)\geq 0$ for all $a\in A$. We also say that $\mathscr{G}$ is the partial compact quantum group \emph{defined by} $\mathscr{A}$.
...
for all $a\in A$,
  \begin{align*}
(\varphi\otimes \id)(\Delta(a)(1\otimes b)) &= \sum_{n}
\varphi(\rho_{n} a)\rho_n b
&& \big(\text{or} &
(\id\otimes \varphi)((b\otimes 1)\Delta(a)) &= \sum_{k}\varphi(\lambda_{k}a)b\lambda_k.\big)
      \end{align*}
\end{Lem}
\begin{proof}
  Straightforward.
...
\begin{Lem}\label{LemFaith} Let $\mathscr{A}$ be a partial Hopf
  algebra with bijective antipode and an integral $\varphi$. Then
  $\varphi$ is faithful in the following sense: if $a\in A$ and
  $\varphi(ab) =0$ (resp. $\varphi(ba)=0$) for all $b\in A$, then
  $a=0$.
\end{Lem} 
...
\begin{proof} Suppose  $\varphi(ba)=0$ for all $b$. Then for all $d\in A$ and all functionals $\omega$ on $A$, the element $p = (\omega\otimes \id)((d\otimes 1)\Delta(a))$ satisfies \[(\id\otimes \varphi)((1\otimes c)\Delta(p)) = 0.\] Continuing as in the proof of \cite[Proposition 3.4]{VDae2}, we obtain from the antipode trick that \[\sum_n \varphi(cS(q)\rho_n)\epsilon(p\lambda_n)=0.\] Choosing now for $c$ and $q$ local units of the form $\lambda_k\rho_l$, the normalization condition on $\varphi$ gives that $\epsilon(p\lambda_n)=0$ for all $n$, hence $\epsilon(p)=0$. This implies $\omega(da)=0$. As $\omega$ and $d$ were arbitrary, it follows that $a=0$.
...
\\
&= \sum_{n} S(b_{(1)})b_{(2)}a_{(1)}\phi(b_{(3)}a_{(2)}) =
S(b_{(1)})\phi(b_{(2)}a)
  \end{align*}
