...
% Semisimplicity strictly says every object _finite_ direct sum of irreducible objects. Should make remark on this... 
...
As in \cite{MVD1}, the  following result will imply that every invariant closed subspace is complemented:
 \begin{Prop}\label{prop:corep-complemented}
   ...
   subspace% Change notation B to C as to not have overlap with notation from first section? 
   \begin{align*}
     B &:= [ \{(h_{km} \otimes
...
We finally show how  Schur's lemma translates into the present context. % Can be formulated as a corollary and I think the proof can be skipped
...
\rho_{n}\xi),
\end{align*}% Some more info, should be applied to elements of the form $\Delta(a)(...)$
...
Lemma \ref{lem:corep-intertwine} implies $S(\Gr{B}{k}{l}{m}{n}) \subseteq \Gr{B}{n}{m}{l}{k}$.
