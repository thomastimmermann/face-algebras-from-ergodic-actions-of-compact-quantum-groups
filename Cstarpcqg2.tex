...
elements of the form % Drop the sum over l in the next equation and the one following it, for clarity
\begin{align*}
  \sum_{l} (\rho_{m}c \zeta^{k}_{l} \otimes \lambda_{m}d\eta),
\end{align*}%Add tildes in next line
...
A$  and   one can check that%probably in next line $m=s$? Also, extra norm of $\xi$ in right hand side
   \begin{align*}
