...
\begin{Def}\label{DefMor} Let $\mathscr{A}$ and $\mathscr{B}$ be respectively $I$ and $J$-partial algebras. A \emph{homomorphism} (based on $\phi$) from $\mathscr{A}$ to $\mathscr{B}$ consists of an assigment  \[\phi: I \ni k \mapsto J_k \subseteq J\] with the $J_k$ disjoint, and linear maps \[\GrDA{f}{r}{s}: A(k\;l)\rightarrow B(r\;s),\quad a\mapsto \GrDA{f(a)}{r}{s}\] for all $r\in I_k, s\in I_l$, satisfying 
\begin{itemize}
\item[$\bullet$] (Unitality) $\GrDA{f(\Unit_{k})}{r}{s} = \delta_{rs}\delta_{r\in I_k}\Unit_r$.
\item[$\bullet$] (Local finiteness) For each $k,l\in I$, $a\in A(k\;l)$ and $r\in J_k$ (resp.~ $s\in J_l$), there exist only finitely many $s\in J_l$ (resp.~ $r\in J_k$) such that $\GrDA{f(a)}{r}{s} \neq 0$. 
...
\end{itemize} 
The homomorphism is called \emph{unital} if $\{J_k\mid k\in I\}$ is a partition of $J$. 
\end{Def}

Note that the multiplicativity condition makes sense because of the local finiteness condition.

\begin{Rem} If $\{J_k\}$ is a partition, we can of course interpret $\phi$ as a map $J\rightarrow I, r\in J_k\mapsto k$. In case $\{J_k\}$ is not a partition, we obtain a function $J\rightarrow I^*$, where $I^*$ is $I$ with an extra point `at infinity' added.

\end{Rem}

The following lemma provides the global viewpoint concerning homomorphisms. 

\begin{Lem} Let $\mathscr{A}$ and $\mathscr{B}$ be respective $I$- and $J$-partial algebras, and fix an assigment $\phi: k\mapsto J_k$. Then there is a one-to-one correspondence between homomorphisms $\mathscr{A}\rightarrow \mathscr{B}$ based on $\phi$ and homomorphism $f:A\rightarrow M(B)$ with $f(\Unit_k) = \sum_{r\in J_k} \Unit_r$. 
\end{Lem} 
\begin{proof}
Straightforward.
\end{proof}

Note that the assigment $k\mapsto J_k$ has to be kept as part of the information since local units are allowed to be zero. 
