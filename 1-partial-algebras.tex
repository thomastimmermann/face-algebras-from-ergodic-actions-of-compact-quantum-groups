...
% Better location?
\begin{Rem} We will call any general assignment $(k,l)\rightarrow m_{kl}$ into a set with a distinguished zero element \emph{row-and column-finite} if the assignment has finite support in either one of the variables when the other variable has been fixed. % More succinct terminology would be banded, but that seems to imply a uniform band
\end{Rem} 
...
\begin{Lem} Let $(\mathscr{A},\Delta)$ be a partial Hopf algebra. Then $\varepsilon\circ S = \varepsilon$ on each $\Gr{A}{k}{l}{m}{n}$.
\end{Lem}
