...
\begin{Prop} Let $(A,\Delta)$ be a left and right full regular weak multiplier bialgebra whose base algebra is isomorphic to $\Fun_f(I)$ for some set $I$, and such that moreover $A^LA^R \subseteq A$. Then $(A,\Delta)$ is the total weak multiplier bialgebra of a uniquely determined partial bialgebra $\mathscr{A}$ over $I$.
\end{Prop} 
...
\begin{proof} Suppose $a\in A$ and $\phi(ba)=0$ for all $b\in A$. We will first show that necessarily $\epsilon(a)=0$. 

Indeed, the condition on $a$ implies also $(\id\otimes \phi)(\Delta(b)(1\otimes a))=0$ for all $b\in A$. Applying the strong invariance identity, we deduce \begin{equation}\label{EqSwitch}(\id\otimes \phi)((1\otimes b)\Delta(a))=0,\qquad \forall b\in A.\end{equation} As in \cite[Proposition 3.4]{VDae2} (and using idempotency of $A$), it follows from this that we also have $0 = \phi(bS(a_{(1)})a_{(2)}) = \phi(b\Pi^R(a)) = \sum_p \phi(b\rho_p)\epsilon(a\rho_p)$.  Taking $b=\UnitC{k}{k}$, we see $\epsilon(a\rho_k)=0$ for all $k$, hence $\epsilon(a)=0$.

Note now that from \eqref{EqSwitch}, it follows that for any functional $\omega$ on $A$ and any $c\in A$, also $a'=(\omega\otimes \id)((c\otimes 1)\Delta(a))$ satisfies $\phi(ba')=0$ for all $b\in A$. Hence, by what we have just shown, $\epsilon(a')=0$, i.e.~ $\omega(ca)=0$. As $\omega$ and $c$ were arbitrary, we deduce $a=0$.
