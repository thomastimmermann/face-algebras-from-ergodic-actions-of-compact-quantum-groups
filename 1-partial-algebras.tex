...
\begin{Def} A \emph{partial algebra} $\mathscr{A}=(\mathscr{A},M)$ (over $\C$) is a set $I$ (the object set) together with 
\begin{itemize}
...
\begin{Rem}
\begin{itemize}\item[$\bullet$] We allow the local units $\Unit_k$ to be zero. %When none of the units are zero, we will call the partial algebra \emph{coconnected}. (Coconnected because we are working contravariantly: we say F(G) (the function algebra) has a property if G has that property. But a partial algebra is the `group algebra' of a linear category, so it's dual gets to be called connected, and the partial algebra itself coconnected)
\item[$\bullet$] A partial algebra is by definition the same as a small $\C$-linear category. However, we do not emphasize this viewpoint, as the natural notion of morphism for partial algebras will be \emph{contravariant} on objects, see Definition \ref{DefMor}.
\end{itemize}
\end{Rem}
...
The subalgebra $A_0$ spanned by all $\Unit_k$ will be called the \emph{base algebra} of $A$. 
\end{Def} 
...
\begin{Def}\label{DefMor} Let $\mathscr{A}$ and $\mathscr{B}$ be respectively $I_A$ and $I_B$-partial algebras. A \emph{(unital) homomorphism} from $\mathscr{A}$ to $\mathscr{B}$ consists of a map \[I_B  \rightarrow I_A, \quad r \mapsto r'\] and maps \[\GrDA{f}{r}{s}: A(r'\;s')\rightarrow B(r\;s),\quad a\mapsto \GrDA{f(a)}{r}{s}\] satisfying 
\begin{itemize}
\item[$\bullet$] (Unitality) $\GrDA{f(\Unit_{r'})}{r}{r} = \Unit_r$.
\item[$\bullet$] (Local finiteness) For each $k,l\in I$, $a\in A(k\;l)$ and $r\in I_B$ (resp.~ $s\in I_B$), there exist only finitely many $s\in I_B$ (resp.~ $r\in I_B$) such that $r'=k$, $s'=l$ and $\GrDA{f(a)}{r}{s} \neq 0$. 
\item[$\bullet$] (Multiplicativity) For all $r,t\in I_B$, all $k\in I_A$ and all $a\in A(r'\;k)$ and $b\in A(k\;t')$, one has \[\GrDA{f(ab)}{r}{t} = \sum_{s,s'=k} \GrDA{f(a)}{r}{s}\GrDA{f(b)}{s}{t}.\]
\end{itemize} 
\end{Def}

Note that the multiplicativity condition makes sense because of the local finiteness condition.

The following lemma provides the global viewpoint concerning morphisms. We will make use of the natural embedding $M(A_0) \subseteq M(A)$. We leave the proof of the lemma to the reader. % Or I'll fill it in when less tired.

\begin{Lem} There is a one-to-one correspondence between morphisms $\mathscr{A}\rightarrow\mathscr{B}$ and homomorphisms $f:A\rightarrow M(B)$ such that $f(A_0)\subseteq M(B_0)$ and $f(A_0)B_0 = B_0$. 
\end{Lem} 

\begin{Rem} For \emph{non-unital morphisms}, one should allow for the map $r\rightarrow r'$ in Definition \ref{DefMor} to be ill-defined at certain indices, in which case the $f$-maps containing such an index should be treated as zero maps. In the multiplier picture, this means one allows for $f$ where one only has $f(A_0)B_0\subseteq B_0$.
\end{Rem}
% Refer to remark in bialgebra section.
...
We want to relate the notion of partial bialgebra to the recently introduced notion of weak multiplier bialgebra \cite{Boh1}. Let us first introduce the following notation, using the notion introduced in Example \ref{ExaMult}.2.
