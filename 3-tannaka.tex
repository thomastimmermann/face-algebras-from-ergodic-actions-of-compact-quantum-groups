...
%Similarly, when $\xi \in F(X)\iitimes F(Y)$, we will write $\xi_{(I)}\otimes \xi_{(J)}$ for the image of $\xi$ under the natural isomorphism $F(X)\iitimes F(Y)\underset{\phi_{X,Y}}{\cong} F(X\otimes Y) \cong \oplus_{a\in I}\left(F(u_a)\otimes \Mor(u_a,X\otimes Y)\right)$.
