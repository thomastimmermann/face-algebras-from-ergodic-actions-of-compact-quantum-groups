\section{C$^*$-algebraic partial compact quantum groups and their representations}
...
%and a straightforward application of Zorn's Lemma
implies the following corollary. % Zorn's lemma not needed. 
...
\rho^{H_{\phi}}_{m} \otimes \rho_{m}$. %Changed wording somewhat 
...
\otimes \lambda_{m}$ as claimed.
 ...
 \begin{Lem}\label{lem:reg-corep-mult}
   ...
   $(\mathcal{K}(H_{\phi}) \otimes A)\tilde V$ lie in %changed order
...
But $\sum_{i}     (|c_{i}\zeta^{k}_{l_{i}}\rangle\langle\xi| \otimes
     d_{i}) \in \mathcal{K}(H_{\phi}) \otimes A$ and again, one can check   that%added norm of $\xi$
     \begin{align*}
...
However,   we do not know whether $(\mathcal{K}(H_{\phi}) \otimes 1)
   \tilde V(1\otimes A)$ is contained in $\mathcal{K}(H_{\phi})\otimes A$.%Changed order
\end{Rem}
...
\end{align*}
%and denote by  $\zeta^{k}_{m}$  the  image of $\UnitC{k}{m}$ in $H_{\phi}$. 
