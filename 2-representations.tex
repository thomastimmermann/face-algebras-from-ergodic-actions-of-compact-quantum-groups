...
Let $\mathscr{A}$ be an $I$-partial bialgebra. 

\begin{Def} Let $\mathscr{A}$ be an $I$-partial bialgebra, and let $\mathcal{H}=\bigoplus_{k,l} \Gru{\mathcal{H}}{k}{l}$ be an $I^{2}$-graded Hilbert space.
A \emph{corepresentation}  $X=(\Gr{X}{k}{l}{m}{n})_{k,l,m,n}$ of $\mathscr{A}$ on $\Hsp$ consists of a family of elements 
...
Let us call an $I^2$-graded Hilbert space $\mathcal{H}=\bigoplus_{k,l} \Gru{\mathcal{H}}{k}{l}$ \emph{separately finitely supported (sfs)} %better name? 
if $\Gru{\Hsp}{k}{l} = \{0\}$ for almost all $k$ (resp. almost all $l$) when $l$ (resp. $k$) is fixed. Similarly, a corepresentation will be called separately finitely supported if its carrier $I^2$-graded Hilbert space is separately finitely supported.

\begin{Def} A \emph{morphism} $T$ between two corepresentations
$(\mathcal{H},X)$ and $(\mathcal{K},Y)$ is a family of bounded linear maps
$\Gru{T}{k}{l} \in
...
\begin{Lem} \label{lemma:rep-corep} Let $\mathscr{A}$ be a partial Hopf algebra, and let $X$ be an sfs corepresentation of $\mathscr{A}$ on an $I^2$-graded Hilbert space $\Hsp$.
  Then the elements % More logical to give the $Z$ the grading of the component in $A$ to which they belong
  \[\Gr{Z}{k}{l}{m}{n} =(S \otimes
  \id)(\Gr{X}{n}{m}{l}{k}) \in
    \Gr{A}{k}{l}{m}{n} \otimes
    \mathcal{B}(\Gru{\mathcal{H}}{l}{k},\Gru{\mathcal{H}}{n}{m})\] are the unique elements such  that
  \begin{align}\label{eq:rep-delta}
    \sum_{k} \Gr{Z}{l}{k}{n'}{m} \Gr{X}{k}{l}{m}{n} &= \delta_{n,n'}
    \UnitC{l}{n} \otimes \id_{\Gru{\mathcal{H}}{m}{n}}, &
    \sum_{n} \Gr{X}{k}{l}{m}{n} \Gr{Z}{l}{k'}{n}{m} &= \delta_{k,k'}
    \UnitC{k}{m}\otimes \id_{\Gru{\mathcal{H}}{k}{l}}.
  \end{align}
...
\end{proof}
 ...
 \begin{Rem} % To see if this has to be modified.
 Using \eqref{eq:rep-delta}, one finds that a collection of bounded operators $\Gru{T}{k}{l} \in
\mathcal{B}(\Gru{\mathcal{H}}{k}{l},\Gru{\mathcal{K}}{k}{l})$ forms a morphism between two corepresentations
$(\mathcal{H},X)$ and $(\mathcal{K},Y)$  if
and only if
  \begin{align} 
...
Let us say an $I^2$-graded Hilbert space $\Hsp$ is seperately finite dimensional (sfd) if $\oplus_l \Gru{\Hsp}{k}{l}$ (resp. $\oplus_l \Gru{\Hsp}{k}{l}$) is finite dimensional for $k$ (resp. $l$) fixed. Correspondingly, we talk of an sfd corepresentation. 

\begin{Def} We denote by $\Corep(A,\Delta)$ the category of sfd corepresentations of $(A,\Delta)$ with bounded morphisms.
\end{Def}

Clearly $\Corep(A,\Delta)$ is a $\C$-linear category. The next Lemma, which is an analogue of Schur's lemma, shows that it is semi-simple.
